...
function computeMapThumb(){const scr=scrollEl.value;if(!scr)return
  ...
  const S=Math.min(M.sH||H,H)               // 缩略图中"有内容"的高度（到加号为止）
  if(S<=0){thumb.top=0;thumb.h=Math.max(8,H);return}
  // 高亮块高度 y = 视口高 b × 缩略比例 r（随窗口变化），不超过内容区
  let y=Math.max(8, scr.clientHeight*(M.r||0.08))
  y=Math.min(y, S)
  if(max<=0){thumb.top=0;thumb.h=y;return}
  thumb.h=y
