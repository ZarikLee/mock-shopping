<template>
  <div class="files-page">
    <div class="fp-head">
      <div>
        <h1>云文件管理</h1>
        <p class="fp-sub">按日期查看、上传或删除该项目的图片与附件</p>
      </div>
      <span class="fp-count">共 {{ totalImg }} 图 · {{ totalFile }} 文件</span>
    </div>

    <div class="quota">
      <span class="q-lbl">云盘</span>
      <div class="q-bar"><i :style="{ width: quotaPct + '%' }"></i></div>
      <span class="q-num">{{ fmtMB(used) }} / {{ fmtMB(limit) }} MB</span>
      <button class="q-buy" @click="convOpen = true">扩容</button>
    </div>
    <p class="quota-hint">基础 100MB；积分扩容：1 积分 = +1MB（永久），当前可用 {{ points }} 积分。</p>

    <div class="fp-tools">
      <div class="filters">
        <button :class="['fc', { on: filter === 'all' }]" @click="filter = 'all'">全部</button>
        <button :class="['fc', { on: filter === 'img' }]" @click="filter = 'img'">图片</button>
        <button :class="['fc', { on: filter === 'file' }]" @click="filter = 'file'">附件</button>
      </div>
      <div class="uploads">
        <input type="date" v-model="upDate" class="up-date" />
        <button class="up-btn" @click="pickImg">传图片</button>
        <button class="up-btn" @click="pickFile">传附件</button>
        <input ref="upImg" type="file" accept="image/*" multiple style="display:none" @change="onImg" />
        <input ref="upFile" type="file" multiple style="display:none" @change="onFile" />
      </div>
    </div>

    <div v-if="loading" class="fp-load"><span class="spin"></span><p>正在整理你的文件…</p></div>
    <div v-else-if="!filtered.length" class="empty">没有{{ filter === 'img' ? '图片' : filter === 'file' ? '附件' : '文件' }}</div>

    <div v-else class="fp-list">
      <section v-for="r in filtered" :key="r.date" class="fp-day">
        <div class="fp-date">{{ dayLabel(r.date) }} <em>{{ r.weekday }}</em>
          <span class="fp-n">{{ r.images.length }} 图 / {{ r.files.length }} 文件</span>
        </div>
        <div v-if="(filter !== 'file') && r.images.length" class="fp-imgs">
          <span v-for="(u, i) in r.images" :key="'i' + i" class="thumb">
            <img :src="u" alt="" @click="open(u, 'image', '图片' + (i + 1))" />
            <i class="rm" @click="delImg(r, i)">×</i>
          </span>
        </div>
        <div v-if="(filter !== 'img') && r.files.length" class="fp-files">
          <button v-for="(f, i) in r.files" :key="'f' + i" class="fp-chip" @click="open(f.url, f.type, f.name)">
            ...
            <i class="rm2" @click.stop="delFile(r, i)">×</i>
          </button>
        </div>
      </section>
    </div>
    ...
    <transition name="fade">
      <div v-if="convOpen" class="conv-mask" @click.self="convOpen = false">
        <div class="conv-card">
          <h3>积分扩容云盘</h3>
          <p class="conv-tip">当前积分：{{ points }}　比例：1 积分 = +1MB（永久）</p>
          <div class="conv-presets">
            <button v-for="p in [10, 30, 60, 100]" :key="p" :class="{ on: convPoints === p }" @click="convPoints = p">{{ p }} 积分 → +{{ p }}MB</button>
          </div>
          <button class="conv-do" :disabled="busy || convPoints < 1 || convPoints > points" @click="doConvert">确认扩容</button>
...
function filterRows(arr) {
  if (filter.value === 'img') return arr.filter(r => r.images.length)
  if (filter.value === 'file') return arr.filter(r => r.files.length)
  return arr
}
function applyFilter() { filtered.value = filterRows(allRows.value) }
const findRow = date => allRows.value.find(r => r.date === date)

async function persistRow(r) {
  const items = (r.items || []).map(i => ({ text: i.text, done: !!i.done }))
  await projectApi.commit(pid.value, r.date, { weekday: wk(r.date), items, files: r.files, images: r.images })
}
function afterChange() { loadStorage() }

async function loadStorage() {
  try { const r = await projectApi.storage(); used.value = r.used || 0; limit.value = r.limit || limit.value; points.value = r.points || 0 } catch {}
}
async function doConvert() {
  if (!convPoints.value || convPoints.value < 1 || convPoints.value > points.value) return
  busy.value = true
  try { await authApi.convertStorage({ points: convPoints.value }); convOpen.value = false; await loadStorage() }
  catch (e) { alert((e && e.error) || '转换失败') } finally { busy.value = false }
}
...
async function load() {
  ...
  try {
    ...
    totalFile.value = allRows.value.reduce((n, r) => n + r.files.length, 0)
    applyFilter()
  } catch { allRows.value = [] } finally { loading.value = false }
  await loadStorage()
}

function pickImg() { upImg.value && upImg.value.click() }
function pickFile() { upFile.value && upFile.value.click() }
async function onImg(e) { await handleFiles(e.target, 'image'); e.target.value = '' }
async function onFile(e) { await handleFiles(e.target, 'file'); e.target.value = '' }
async function handleFiles(input, kind) {
  const arr = Array.from(input.files)
  if (!arr.length) return
  const date = upDate.value || tNowStr()
  let r = findRow(date)
  if (!r) { r = { date, weekday: wk(date), items: [], files: [], images: [] }; allRows.value.unshift(r) }
  const cap = kind === 'image' ? 3 * 1024 * 1024 : 8 * 1024 * 1024
  const reads = arr.map(f => new Promise(res => { if (f.size > cap) return res(null); const rd = new FileReader(); rd.onload = () => res(kind === 'image' ? String(rd.result) : { name: f.name, type: f.type || '', url: String(rd.result) }); rd.onerror = () => res(null); rd.readAsDataURL(f) }))
  const ok = await Promise.all(reads)
  const got = ok.filter(Boolean)
  if (!got.length) { alert('文件过大（图片≤3MB/附件≤8MB）或读取失败'); return }
  if (kind === 'image') r.images.push(...got); else r.files.push(...got)
  try { await persistRow(r); await load() } catch (e) { alert('保存失败，可能超出云盘容量：' + (e.error || e.message || '')) }
}
async function delImg(r, i) { r.images.splice(i, 1); try { await persistRow(r); await load() } catch (e) { alert('删除失败') } }
async function delFile(r, i) { r.files.splice(i, 1); try { await persistRow(r); await load() } catch (e) { alert('删除失败') } }

function tNowStr() { const d = new Date(); return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}` }
watch(filter, () => { filtered.value = filterRows(allRows.value) })
onMounted(() => { if (!user.isLoggedIn) { router.push('/login'); return } upDate.value = tNowStr(); load() })
</script>
...
.fp-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; margin-bottom: 16px; }
...
.quota { display: flex; align-items: center; gap: 10px; background: var(--surface); border: 1px solid var(--border); border-radius: 12px; padding: 10px 14px; margin-bottom: 6px; flex-wrap: wrap; }
.q-lbl { font-size: 12px; color: var(--text-2); }
.q-bar { flex: 1; min-width: 120px; height: 8px; border-radius: 5px; background: var(--surface-2); overflow: hidden; }
...
.quota-hint { font-size: 11px; color: var(--text-2); margin: 0 0 14px; }
.fp-tools { display: flex; align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap; margin-bottom: 14px; }
.filters { display: flex; gap: 6px; }
.fc { border: 1px solid var(--border); background: var(--surface); color: var(--text-2); border-radius: 999px; padding: 5px 14px; font-size: 12px; cursor: pointer; }
.fc.on { background: var(--accent); color: #fff; border-color: var(--accent); }
.uploads { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.up-date { padding: 5px 8px; border: 1px solid var(--border); border-radius: 9px; background: var(--surface); color: var(--text); font-size: 12px; }
.up-btn { border: 1px dashed var(--border); background: transparent; color: var(--text-2); border-radius: 9px; padding: 5px 12px; font-size: 12px; cursor: pointer; }
.up-btn:hover { border-color: var(--accent); color: var(--accent); }
.empty { text-align: center; color: var(--text-2); padding: 60px 0; }
.fp-load { display: flex; flex-direction: column; align-items: center; gap: 14px; padding: 80px 0; color: var(--text-2); font-size: 13px; }
...
.fp-imgs .thumb { position: relative; width: 96px; height: 96px; border-radius: 10px; overflow: hidden; border: 1px solid var(--border); cursor: zoom-in; }
.fp-imgs .thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
.fp-imgs .thumb .rm { position: absolute; top: 3px; right: 3px; width: 17px; height: 17px; border-radius: 50%; background: rgba(0,0,0,.55); color: #fff; font-style: normal; font-size: 12px; line-height: 16px; text-align: center; cursor: pointer; opacity: 0; transition: opacity .15s; }
.fp-imgs .thumb:hover .rm { opacity: 1; }
.fp-files { display: flex; flex-wrap: wrap; gap: 6px; }
.fp-chip { position: relative; display: inline-flex; align-items: center; gap: 5px; max-width: 260px; border: 1px solid var(--border); background: var(--bg); color: var(--text); font-size: 12px; padding: 5px 10px; border-radius: 8px; cursor: pointer; }
.fp-chip svg { fill: none; stroke: currentColor; stroke-width: 1.8; stroke-linecap: round; stroke-linejoin: round; flex-shrink: 0; }
.fc-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.rm2 { width: 15px; height: 15px; border-radius: 50%; background: rgba(0,0,0,.45); color: #fff; font-style: normal; font-size: 11px; line-height: 14px; text-align: center; cursor: pointer; opacity: 0; transition: opacity .15s; flex-shrink: 0; }
.fp-chip:hover .rm2 { opacity: 1; }
...
.pv-mask { position: fixed; inset: 0; background: rgba(0,0,0,.55); z-index: 260; display: flex; align-items: center; justify-content: center; padding: 24px; }
