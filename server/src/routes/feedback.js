...
router.get('/', authMiddleware, async (req, res, next) => {
  try {
    let rows = (await queryAll('feedback')).sort((a, b) => b.createdAt - a.createdAt);
...
router.post('/', authMiddleware, async (req, res, next) => {
  try {
    ...
    const rec = await insert('feedback', {
      userId: req.user.id,
...
router.post('/:id/reply', authMiddleware, async (req, res, next) => {
  try {
    ...
    const f = await queryOne('feedback', { id });
    ...
    const updated = await update('feedback', id, { replies });
